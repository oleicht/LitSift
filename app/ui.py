...
class AlternatingListbox(tk.Listbox):
    def __init__(self, master=None, **kwargs):
        ...
        self.configure(selectbackground='#e0e0e0')  # Set selection background to match alternating color
...
def create_main_window():
    ...
    submit_button = tk.Button(root, text="Submit", command=lambda: on_submit(entry, result_listbox, root), font=default_font)
    ...
    result_listbox = AlternatingListbox(list_frame, height=scaled(10), width=scaled(50), font=default_font, yscrollcommand=scrollbar.set)
...
def show_description(root, title, description):
    ...
    download_button = tk.Button(button_frame, text="Download", command=download_content, 
        ...
        if input_text:
            ...
            def update_ui():
                ...
                for i, item in enumerate(result):
                    ...
                    if i % 2 == 0:
                        result_listbox.itemconfigure(i, background='#f0f0f0')  # Light gray
                    else:
                        result_listbox.itemconfigure(i, background='#e0e0e0')  # Slightly darker gray
                    root.result_data.append((title, description))
                
            root.after(0, update_ui)
        else:
            root.after(0, lambda: result_listbox.delete(0, tk.END))
            root.after(0, lambda: result_listbox.insert(tk.END, "Please enter some text."))
