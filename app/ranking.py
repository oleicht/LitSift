...
def get_data():
    ...
    all_papers = client.get_all_notes(content={'venueid': config["openreview"]["venue"]})
    fields = ["title", "authors", "abstract", "venue", "pdf"] # ,"keywords",] 
    extracted = [(paper.to_json()["id"], ) + tuple(paper.to_json()["content"].get(f, {"value": ["n/a"] })["value"]
                       for f in fields) for paper in all_papers]
...
def generate_embeddings(model=None):
    ...
    venue_str = config["openreview"]["venue"].replace(".", "").replace("/", "-")
    cached_embeddings_file = Path(__file__).parent / "cache" / f"{venue_str}/{config['ranking']['model']}-embeddings.parquet"
    if cached_embeddings_file.exists():
        return pd.read_parquet(cached_embeddings_file)
    
    data = get_data()
    paper_strings = data.apply(lambda row: "Title: "+ row.title + " Abstract: " + row.abstract, axis=1)
    
    if config["ranking"]["model"]=="voyage-large-2-instruct":
        df = generate_voyageai_embeddings_robustly(paper_strings, cached_embeddings_file)
...
def generate_voyageai_embeddings_robustly(paper_strings, cached_embeddings_file):
    ...
    for i, tb in tqdm(enumerate(text_blocks), total=len(text_blocks)):
        ...
        try:
            ...
            base = pd.concat([pd.DataFrame({"range": range(*tb),
                                            "title_abstract": papers,
                                        }),
                            pd.DataFrame(np.array(res.embeddings, dtype=np.float32), columns=[f"emb{jj}" for jj in range(1024)])],
                            axis=1)
    ...
    return dfc.set_index("range").sort_index().rename(columns={"title_abstract": "paper_strings"})
...
def get_rankings(query):
    if config["ranking"]["model"]=="all-mpnet-base-v2":
        ...
        x_embeddings = embeddings[[c for c in embeddings.columns if c != "paper_strings"]].to_numpy()
        ...
        x_embeddings = embeddings[[c for c in embeddings.columns if c != "paper_strings"]].to_numpy()
        paper_strings = embeddings[["paper_strings"]]
        latent_query = np.array(vo.embed([query],
                                         model="voyage-large-2-instruct",
                                         input_type="query").embeddings,
                                dtype=np.float32)
        # note: empirically, the voyage-ai embeddings seems to have norm=1
        qk = (- latent_query @ x_embeddings.T / (norm(x_embeddings, axis=1) * norm(latent_query, axis=1)))[0]
    
    preferences = qk.argsort()
    return [(row.paper_strings.split(" Abstract: ")[0][len("Title: "):],
             row.paper_strings.split(" Abstract: ")[1]) for row
            in paper_strings.iloc[preferences].itertuples() if row.paper_strings is not None]
